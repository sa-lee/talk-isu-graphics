...
.vlarge[_Linear_: $Y$ is a linear transformation of the input $X$, i.e. principal components analysis (PCA).
...
.vlarge[**t-distributed stochastic neighbour embedding**
...
.vlarge[_Key idea:_ transform pairwise $\mathbf{x_i}$ and $\mathbf{x_j}$ 
...
.vlarge[The variance $\sigma_{i}^2$ of the Gaussian is chosen based on user supplied value of perplexity:
...
.vlarge[**Grand Tour:** generate a set of random orthnormal bases, interpolate between them.]
...
.vlarge[The tour is enhanced by **interactivity**:
...
.vlarge[
- vegalite increases code complexity
